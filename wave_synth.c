...
void wave_synth_init(pocket_piano *pp6){
	uint8_t i;
	for (i = 0; i<4; i++){
		bl_saw_reset(&(pp6->saws[i]));
		bl_square_init(&(pp6->squares[i]));
	}
...
float wave_synth_process(pocket_piano *pp6){
	...
	if (pp6->aux_button_count == 0){
		...
		sig = 0;
		for (i = 0; i < 4; i++) {
			sin_set(&(pp6->sins[i]), pp6->freqs[i] * (pp6_get_knob_1(pp6) + 1), .1f);
			sig += sin_process(&(pp6->sins[i]), pp6->wavetable_selector) * pp6->amps[i];
		}
		sig *= 1;
	}
	...
	if (pp6->aux_button_count == 3){
		sig = 0;
		for (i = 0; i < 4; i++) {
			bl_square_set(&(pp6->squares[i]), pp6->freqs[i] * (pp6_get_knob_1(pp6) + 1) );
			sig += bl_square_process(&(pp6->squares[i])) * pp6->amps[i];
		}
		sig *= .2f;
	}
	if (pp6->aux_button_count == 4){
		sig = 0;
		for (i = 0; i < 4; i++) {
			bl_saw_set(&(pp6->saws[i]), pp6->freqs[i] * (pp6_get_knob_1(pp6) + 1) );
			sig += bl_saw_process(&(pp6->saws[i])) * pp6->amps[i];
		}
		sig *= .2f;
	}
